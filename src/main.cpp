	...
	if (state == State::Gameplay) {
		...
		if (!player.buyingShield && !player.buyingShotgun && !pausing && !gameOver) {
			...
			if (portalClock.getElapsedTime() > PORTAL_SPAWN_DELAY_IN_MS && portalRects.size() < 1) {
				...
				portalRects.back().x = MathUtils::Random(0, windowWidth - portalRects.back().w);
				portalRects.back().y = MathUtils::Random(0, windowHeight - portalRects.back().h);
				portalRects.push_back(SDL_FRect());
				portalRects.back().w = 32;
				portalRects.back().h = 32;
				portalRects.back().x = MathUtils::Random(0, windowWidth - portalRects.back().w);
...
void InputEvents(const SDL_Event& event) {
	...
	if (event.type == SDL_MOUSEBUTTONDOWN) {
		...
		if (gameOver) {
			...
			if (SDL_PointInFRect(&mousePos, &closeBtnR) && (player.buyingShield || player.buyingShotgun)) {
				player.buyingShield = false;
				player.buyingShotgun = false;
				menuSuccess = true;
			}
			if (SDL_PointInFRect(&mousePos, &buyBtnR) && player.buyingShield) {
			...
			if (SDL_PointInFRect(&mousePos, &buyShotgunBtnR) && player.buyingShotgun) {
...
void EnemySpawn() {
	if (enemyClock.getElapsedTime() > ENEMY_SPAWN_DELAY_IN_MS) {
		...
		case SpawnPlace::Up:
			enemies.back().r.x = MathUtils::Random(0, windowWidth - enemies.back().r.w);
			enemies.back().r.y = 0;
			enemies.back().dy = 1;
			break;
		...
		case SpawnPlace::Left:
			...
			enemies.back().r.y = MathUtils::Random(0, windowHeight - enemies.back().r.h);
			enemies.back().dx = 1;
			break;
...
void EntityMovement(const SDL_FRect& extendedWindowR) {
	...
	//Player movement
	player.MoveBehaviour(windowWidth, windowHeight);
	//Enemy movement
	Enemy::MoveEnemyGroup(&enemies);
	//Planet's movement
	Planet::MovePlanetGroup(&planets, &player, extendedWindowR, SDL_HasIntersectionF);
...
void PowerUpSpawner()
{
	if (planetClock.getElapsedTime() > PLANET_SPAWN_DELAY_IN_MS && !hasShield) {
		PlanetType planetType = Planet::RandomPlanetType(hasShotgun);
		planets.push_back(Planet(planetType, windowWidth));
		planetClock.restart();
	}
	...
	if (healthPickupClock.getElapsedTime() > healthSpawnTime) {
		...
		healthPickups.back().r.x = MathUtils::Random(0, windowWidth - healthPickups.back().r.w);
	...
	if (!player.hasBomb && player.streak >= STREAK_BOMB_REQUIREMENT && bombs.size() < 1) {
		...
		bombs.back().r.w = 64;
		bombs.back().r.h = 64;
		bombs.back().r.x = MathUtils::Random(0, windowWidth - bombs.back().r.w);
...
void RenderAll() {
	...
	for (int i = 0; i < planets.size(); ++i) {
		switch (planets[i].GetPlanetType()) {
